...
Web actions are OpenWhisk actions annotated to quickly enable you to build web based applications. This allows you to program backend logic that your
web application can access anonymously without requiring an OpenWhisk authentication key. It is up to the action author to implement their own desired authentication and authorization (i.e. OAuth flow). 
...
Create a _web action_ `hello` in the package `demo` for the namespace `guest` using the annotation `web-export`
```bash
$ wsk package create demo
$ wsk action create /guest/demo/hello hello.js -a web-export true
```

The `web-export` annotation allows the action to be accessible as a web action via a new REST interface `https://APIHOST/api/v1/experimental/web/` followed by the fully qualified name of the action which must include its namespace, package name, or default if the action is not in a named package. The web action API path may be used with curl or wget without an API key. It may even be entered directly in your browser.

Now the action is accesible on the URL:
```
https://${APIHOST}/api/v1/experimental/web/{namespace}/{packageName}/{actionName}
``` 

Try opening [https://${APIHOST}/api/v1/experimental/web/guest/demo/hello.html?name=Jane](https://${APIHOST}/api/v1/experimental/web/guest/demo/hello.html?name=Jane) on your web browser.

Lets try invoking the action via curl:
```
$ curl https://${APIHOST}/api/v1/experimental/web/guest/demo/hello.html?name=Jane
```
...
To disable a web action from being invoke via `https://APIHOST/api/v1/experimental/web/` API, it’s enough to remove the annotation or set it to `false`.
